...
var GooglePayPaymentProvider = class {
  ...
  async createPayment(params) {
    ...
    return {
      ...
      currency: params.currency,
      status: "PENDING",
      instructions: config2.isTestMode ? "TEST MODE: Google Pay 1-tap sheet tokenization simulation (no real card charged)." : "Google Pay encrypted payment token request.",
      isTestMode: config2.isTestMode
    };
  }
  async verifyPayment(paymentId, providerTransactionId, simulateAction = "success") {
    const config2 = getPaymentConfig();
...
var ApplePayPaymentProvider = class {
  ...
  async createPayment(params) {
    ...
    return {
      ...
      currency: params.currency,
      status: "PENDING",
      instructions: config2.isTestMode ? "TEST MODE: Apple Pay merchant session & biometric Touch/Face ID simulated verification." : "Apple Pay merchant validation session.",
      isTestMode: config2.isTestMode
    };
  }
  async verifyPayment(paymentId, providerTransactionId, simulateAction = "success") {
