...
export function getRealisticSkills(position: string): string[] {
    ...
    if (typedList.length > 0) {
      ...
      identifiedPlayers = typedList.map((tp, idx) => {
        ...
        return {
          ...
          confidenceLevel: 'VERIFIED',
          status: 'verified',
          playerType: tp.cardType || (tp as any).playerType || masterMatch?.cardType || 'Highlight',
          cardType: tp.cardType || (tp as any).playerType || masterMatch?.cardType || 'Highlight',
          ...
          evidence: [
            ...
            `Position: ${(tp.position || masterMatch?.primaryPosition || 'CMF').toUpperCase()}`,
            `Card Type: ${tp.cardType || (tp as any).playerType || masterMatch?.cardType || 'Highlight'} (${tp.rating || masterMatch?.maxRating || 90} OVR)`,
