...
export class PesapalPaymentProvider implements PaymentProvider {
  ...
  async createPayment(params: CreatePaymentParams): Promise<CreatePaymentResult> {
    ...
    if (!token) {
      const reason = this.lastAuthError
        ? `Pesapal authentication error (${this.lastAuthError}). Please verify your Pesapal Merchant API 3.0 Consumer Key & Secret.`
        : 'Failed to authenticate with Pesapal API 3.0.';
      return {
        paymentId,
        provider: this.name,
        amount: params.amount,
  ...
  async verifyPayment(
    ...
    // Live Pesapal API verification strictly requiring authenticated token and valid tracking ID
    const token = await this.getAuthToken();
    if (!token) {
      return {
        paymentId,
